...
import { sparkpostLabs as sparkpostLabsAxios } from 'src/helpers/axiosInstances';
...
function redispatchAfterRefresh(action, dispatch) {
  return resolveOnCondition(() => !refreshing).then(() => dispatch(sparkpostLabsRequest(action)));
}

const sparkpostLabsRequest = requestHelperFactory({
  request: sparkpostLabsAxios,
    ...
    if (response.status === 401 && auth.refreshToken && retries < maxRefreshRetries) {
      ...
      return useRefreshToken(auth.refreshToken)
        ...
        .then(
          ...
          () => {
            refreshing = false;
            return dispatch(sparkpostLabsRequest(action));
          },
